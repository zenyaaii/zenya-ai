...
const FS = [
  ...
  "    band(a, al, q, fan, -0.40, 0.070, 1.10, 0.150, 0.0, 0.150, 0.05, uAcc, t);",
  "    band(a, al, q, fan, -0.20, 0.085, 0.95, 0.185, 1.1, 0.170, 0.30, uAcc, t);",
  "    band(a, al, q, fan,  0.00, 0.075, 1.25, 0.135, 2.3, 0.160, 0.55, uAcc, t);",
  "    band(a, al, q, fan,  0.20, 0.090, 1.05, 0.205, 3.4, 0.175, 0.75, uAcc, t);",
  "    band(a, al, q, fan,  0.42, 0.095, 0.90, 0.125, 5.7, 0.185, 0.95, uAcc, t);",
  "    col = a / max(al, 0.0001);",
  ...
  "    col = mix(shade(uAcc, 0.45), vec3(1.0), core * 0.55 + head * 0.25);",
...
export default function DeckLine({
  ...
  panel: number
  style: LineStyle
  /* Only the screen you are on runs a frame loop. The others hold one frame. */
  active: boolean
}) {
  ...
  useEffect(() => {
    ...
    const acc = ACCENT[panel] ?? ACCENT[0]
    gl.uniform3f(uAcc, acc[0] / 255, acc[1] / 255, acc[2] / 255)
    gl.uniform1f(uDark, DARK[panel] ?? 0)
    ...
    if (reduce) {
      ...
      const tick = (now: number) => {
        ...
        /* Off-screen panels keep their last frame rather than burning GPU on a
           screen nobody is looking at. */
        if (!activeRef.current) return
        paint(((now - t0) / 1000) * 0.16)
      }
      /* One frame immediately so a panel is never blank when it arrives. */
      paint(0.4)
      raf = requestAnimationFrame(tick)
    }
