...
/**
 * LiveUsersGlobe — the interactive 3D dot-globe at the foot of the homepage,
 * cropped ~40% into the footer. Continents are an indigo dot cloud; every
 * country with someone online now (real data from /api/live-users) gets a
 * pulsing orange dot, and a live count is shown above.
 *
 * Reliability: Three.js is imported STATICALLY (bundled with the page, so there
 * is no runtime chunk to fail to fetch) and the land is baked into
 * lib/land-dots.ts (no map download at runtime). Earlier a dynamic import() of
 * three + a fetch of the world map were failing in production, leaving a blank
 * canvas; both runtime fetches are now gone.
 */
...
export default function LiveUsersGlobe() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        countriesRef.current = j.countries || []
        setData(j)
  ...
  useEffect(() => {
    ...
    if (countriesRef.current.length) setMarkers(countriesRef.current)
    ...
    <section className="zg-section" aria-labelledby="zg-heading">
      <div className="zg-head">
        <div className="zg-live-badge">
          <span className="zg-dot" data-idle={hasData && activeNow === 0 ? '' : undefined} />
          {!hasData ? (
      ...
      <div className="zg-stage" ref={stageRef}>
        <div className="zg-holder" ref={holderRef}>
          <canvas ref={canvasRef} className="zg-globe" aria-label="كرة أرضية تفاعلية، كل نقطة برتقالية شخص متصل بزينيا الآن" />
        </div>
